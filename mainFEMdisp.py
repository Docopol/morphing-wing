...
def nodeDistance(data_file:np.ndarray)->np.ndarray:
    ...
    for data_file_row in range(0, nr_rows-1):
        dx = (data_file[data_file_row+1, 1]+data_file[data_file_row+1, 4])-(data_file[data_file_row, 1]+data_file[data_file_row, 4])
        dy = (data_file[data_file_row + 1, 2] + data_file[data_file_row + 1, 5])-(data_file[data_file_row, 2] + data_file[data_file_row, 5])
        node_distance[data_file_row, 0] = np.sqrt(dx**2+dy**2)

    return node_distance

loadstep1_disp = reformatFile(readFile("Data/FEM/shell_loadstep1_disp.out"))
loadstep2_disp = reformatFile(readFile("Data/FEM/shell_loadstep2_disp.out"))
loadstep3_disp = reformatFile(readFile("Data/FEM/shell_loadstep3_disp.out"))
loadstep4_disp = reformatFile(readFile("Data/FEM/shell_loadstep4_disp.out"))
loadstep5_disp = reformatFile(readFile("Data/FEM/shell_loadstep5_disp.out"))
...
plt.scatter(loadstep1_disp[:, 1]+loadstep1_disp[:, 4], loadstep1_disp[:, 2]+loadstep1_disp[:, 5], label = "Loadstep 1")
plt.scatter(loadstep2_disp[:, 1]+loadstep2_disp[:, 4], loadstep2_disp[:, 2]+loadstep2_disp[:, 5], label = "Loadstep 2")
plt.scatter(loadstep3_disp[:, 1]+loadstep3_disp[:, 4], loadstep3_disp[:, 2]+loadstep3_disp[:, 5], label = "Loadstep 3")
plt.scatter(loadstep4_disp[:, 1]+loadstep4_disp[:, 4], loadstep4_disp[:, 2]+loadstep4_disp[:, 5], label = "Loadstep 4")
plt.scatter(loadstep5_disp[:, 1]+loadstep5_disp[:, 4], loadstep5_disp[:, 2]+loadstep5_disp[:, 5], label = "Loadstep 5")
